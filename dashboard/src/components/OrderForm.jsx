...
export default function OrderForm({ onCreated }) {
  ...
  async function submit(event) {
    ...
    try {
      const response = await fetch('/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, quantity: Number(form.quantity) }) });
      const body = await response.json();
      if (!response.ok || !body.success) throw new Error(body.error?.message || 'Order could not be created');
      setState({ busy: false, error: '', success: `Order ${body.data.id.slice(0, 8)} created` }); setForm({ productId: '', customerId: '', quantity: 1 }); onCreated?.();
  ...
  return <form className="card" onSubmit={submit}><h3>Create an order</h3>
    <div className="field"><label htmlFor="productId">Product UUID</label><input required id="productId" name="productId" value={form.productId} onChange={update} placeholder="e.g. 8c..." /></div>
    <div className="field"><label htmlFor="customerId">Customer UUID</label><input required id="customerId" name="customerId" value={form.customerId} onChange={update} placeholder="e.g. a4..." /></div>
